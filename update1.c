...
void lval_print_str(lval* v){
    ...
    escaped=mpcf_escape(escaped);// this is the inbuilt mpc function
    printf("\"%s\"",escaped);
...
void lval_print(lval* v){
    switch(v->type){
        ...
        case LVAL_STR: lval_print_str(v);break;//predefined mpc functions
        case LVAL_FUN: 
            if(v->builtin){
                printf("<builtin>");
            }
...
lval* builtin_lambda(lenv* e,lval* a){
    ...
    for(int i=0;i<a->cell[0]->count;i++){
        ...
        "Cannot define non_symbol.Got %s ,Expexted %s ",
...
int main(int argc,char** argv){
    ...
    if(argc>=2){
        //loop over each supplied filename (Strating from 1)
